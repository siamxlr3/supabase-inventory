...
import { useGetSupplierByIdQuery, useUpdateSupplierMutation, useDeleteSupplierMutation } from '@/store/api/supplierApi';
...
export default function EditSupplierPage() {
  ...
  const { id } = useParams();
  const { data: supplierResponse, isLoading: isLoadingSupplier } = useGetSupplierByIdQuery(id as string, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await updateSupplier({ id: id as string, body: formData }).unwrap();
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete this supplier?')) {
      try {
        await deleteSupplier(id as string).unwrap();
  ...
  if (isLoadingSupplier) return <div className="flex h-64 items-center justify-center"><Loader size="lg" /></div>;

  return (
    <>
      <PageHeader
        title={`Edit Supplier: ${formData.name}`}
        description="Update contact information and financial terms"
        breadcrumbs={[
